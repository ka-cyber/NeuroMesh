...
from models.layers import GraphConvLayer, NeuroMeshLayer
from models.segmentation import NeuroMeshUNet
from models.baselines import DropoutUNet, EnsembleUNet, _PlainUNet
from utils.loss import dice_loss, task_loss, rewiring_loss, sparsity_loss, diversity_loss, NeuroMeshLoss
from utils.evaluation import dice_per_class, apply_modality_dropout
from utils.failure_models import random_failure, correlated_failure, cascading_failure, byzantine_failure
from data.brats_loader import (
    MockBraTSDataset, build_dataloader, BraTSDataset,
    build_peds_crosswalk, discover_peds_site_raw_cases, BRATS_PRESETS,
)
...
def test_cascading_failure_is_monotonically_non_decreasing_in_corruption():
    from utils.failure_models import cascading_failure_sequence
...
# -------------------------------------------- peds metadata crosswalk pairing

def _write_fake_metadata_tsv(path):
    with open(path, "w", newline="") as f:
        f.write("BraTS-SubjectID\tMappingID\tAge at imaging (days)\n")
        f.write("BraTS-PED-00001-000\tC1036890\t4545\n")
        f.write("BraTS-PED-00002-000\tC9999999\t1000\n")


def test_build_peds_crosswalk_resolves_known_case(tmp_path):
    tsv_path = tmp_path / "metadata.tsv"
    _write_fake_metadata_tsv(tsv_path)
    by_mapping_and_age, by_mapping_only = build_peds_crosswalk(str(tsv_path))
    assert by_mapping_and_age[("C1036890", "4545")] == "BraTS-PED-00001-000"
    assert by_mapping_only["C1036890"] == "BraTS-PED-00001-000"


def test_discover_peds_site_raw_cases_pairs_real_naming_pattern(tmp_path):
    tsv_path = tmp_path / "metadata.tsv"
    _write_fake_metadata_tsv(tsv_path)

    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    for modality in ["T1", "T1CE", "T2", "FL"]:
        (raw_dir / f"C1036890_4545_{modality}_to_SRI_defaced.nii").touch()
    (raw_dir / "BraTS-PED-00001-000-seg.nii").touch()

    cases = discover_peds_site_raw_cases(str(raw_dir), str(tsv_path), str(raw_dir))
    assert len(cases) == 1
    case = cases[0]
    assert case["subject_id"] == "BraTS-PED-00001-000"
    assert all(k in case for k in ("t1", "t1ce", "t2", "flair", "seg"))
    assert case["t1"].endswith("C1036890_4545_T1_to_SRI_defaced.nii")


def test_discover_peds_site_raw_cases_skips_incomplete_case(tmp_path, capsys):
    tsv_path = tmp_path / "metadata.tsv"
    _write_fake_metadata_tsv(tsv_path)

    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    # only 2 of 4 modalities present -> should be skipped, not silently dropped
    (raw_dir / "C1036890_4545_T1_to_SRI_defaced.nii").touch()
    (raw_dir / "C1036890_4545_T2_to_SRI_defaced.nii").touch()
    (raw_dir / "BraTS-PED-00001-000-seg.nii").touch()

    cases = discover_peds_site_raw_cases(str(raw_dir), str(tsv_path), str(raw_dir))
    assert len(cases) == 0
    assert "missing modalities" in capsys.readouterr().out


def test_brats_dataset_from_case_list_loads_correct_shapes(tmp_path):
    import numpy as np
    ...
    for modality in ["t1", "t1ce", "t2", "flair"]:
        ...
        case[modality] = str(path)
    seg = np.zeros(shape, dtype=np.float32)
    seg[4:8, 4:8, :] = 2
    seg_path = tmp_path / "seg.nii.gz"
    nib.save(nib.Nifti1Image(seg, affine), str(seg_path))
    case["seg"] = str(seg_path)

    ds = BraTSDataset.from_case_list([case], slice_axis=2, target_size=(16, 16))
    assert len(ds) == 4  # 4 slices along axis 2
    x, y = ds[0]
    assert x.shape == (4, 16, 16)
    assert y.shape == (16, 16)
    assert set(y.unique().tolist()) <= {0, 2}


def test_preset_num_classes_metadata_is_consistent():
    assert BRATS_PRESETS["adult"]["num_classes"] == 4
    assert BRATS_PRESETS["peds"]["num_classes"] == 5
