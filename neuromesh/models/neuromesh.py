...
class NeuroMeshUNet(nn.Module):
    ...
    def forward(self, x, h_prev=None, failure_signal=None, return_debug=False):
        """
        Args:
            ...
            return_debug: if True, also return a dict of intermediate controller
               tensors (pooled bottleneck vector, failure_signal used, gate,
               effective adjacency) for mechanism analysis. Does NOT change any
               computed value when False (default) -- purely additive
               instrumentation, added post-hoc for inspecting an already-frozen
               checkpoint, never used during training of that checkpoint.
        Returns:
            logits: [B, num_classes, H, W]
            h_new:  updated controller hidden state
            mask:   predicted bottleneck edge-activation mask (for loss terms / logging)
            debug:  (only if return_debug=True) dict with keys 'pooled',
                    'failure_signal', 'gate', 'effective_adj'
        """
        x1 = self.inc(x)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        ...
        if return_debug:
            effective_adj = mask * self.controller.A_learnable.unsqueeze(0)
            debug = {
                "pooled": pooled.detach(),
                "failure_signal": failure_signal.detach(),
                "gate": gate.detach().view(B, C),
                "effective_adj": effective_adj.detach(),
            }
            return logits, h_new, mask, debug
